...
contract FactoryDraw {
    ...
    function createNewDraw (bytes32 drawId, uint _launchTime, uint _duration) public {
      if (_duration == 15) {
        ...
        uint _launchTimeInSec = block.timestamp + _launchTime * 60 * 60 * 24;
    ...
    function addNewPlayer (uint256 _stake, uint256 _btcPrediction, uint _duration ) public {
      if (_duration >=0 && _duration <= 15 && currentDraw_15 >= 0) {
        Draws_15[uint(currentDraw_15)].addNewPlayer(msg.sender, _stake, _btcPrediction);
      }

      if (_duration > 15 && _duration <= 30 && currentDraw_30 >= 0) {
        Draws_30[uint(currentDraw_30)].addNewPlayer(msg.sender, _stake, _btcPrediction);
      }

      if (_duration > 30 && _duration <= 60 && currentDraw_15 >= 0) {
        Draws_60[uint(currentDraw_60)].addNewPlayer(msg.sender, _stake, _btcPrediction);
      }
    }
}