    ...
    return (
        ...
        >
            <ModalHeader>
                ...
                <Icon url={`close-white.svg`} height={15} width={15} cursor={'pointer'} onClick={props.close}></Icon>
            </ModalHeader>
            <Row width={'100%'} padding={'0 50px'}>
                <Block align={'flex-start'}>
                    <h3>{t('Your prediction')}</h3>
                    <p>{t('This pool will mature at')}:</p>
                    <h2>{moment(new Date()).format('DD/MM/YY')} at {moment(new Date()).format('HH:mm')}</h2>
                </Block>
                <Block align={'center'}>
                    <PredictionRange>
                        <h4>{t('What will the price of bitcoin be when this pool matures?')}</h4>
                        <Row width={'100%'}>
                            <p>{t('prediction')}</p>
                            <p>{t('range')}</p>
                        </Row>
