...
export function ChartWithBet({totem, startValue, betValue, data}: Props) {
    const [maxValue, setMaxValue] = useState(Math.max(...data.slice(
        data.length - TotemsData[totem].days * COUNT_OF_TICKS_FIRST_GRAPH,
        data.length - 1,
    ).map(v => +v.close)) * 1.5);
    const [minValue, setMinValue] = useState(Math.min(...data.slice(
        data.length - 1 - TotemsData[totem].days * COUNT_OF_TICKS_FIRST_GRAPH,
        data.length - 1,
    ).map(v => +v.close)) * 0.5);
    const [stepY, setStepY] = useState((maxValue - minValue) / COUNT_OF_HORIZONTAL_LINES);
            ...
            >
                ...
                <XAxis
                    ...
                    tickFormat={v =>
                        `${moment()
                            .subtract(
                                TotemsData[totem].days * COUNT_OF_TICKS_FIRST_GRAPH - TotemsData[totem].days * v,
                                'd',
                            )
                            .format('DD.MM')}`
                    }
